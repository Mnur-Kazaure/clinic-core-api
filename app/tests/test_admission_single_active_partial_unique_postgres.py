...
from app.shared.enums import AdmissionStatus, AdmissionType, Gender
...
def test_single_active_admission_partial_unique_index():
    ...
    db = SessionLocal()
    try:
        clinic = Clinic(id=uuid.uuid4(), name="Admission Unique Clinic")
        db.add(clinic)
        db.commit()
        ...
        patient = Patient(
            id=uuid.uuid4(),
            clinic_id=clinic.id,
            full_name="Admission Unique Patient",
