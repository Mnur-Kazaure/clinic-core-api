...
if [[ -z "${PGPASSWORD:-}" ]]; then
  ...
  echo "Example: PGPASSWORD=postgres ./scripts/reset-isolated-db.sh"
  exit 1
fi

if [[ ! -x "$ROOT_DIR/.venv/bin/alembic" ]]; then
  echo "Missing Alembic executable at .venv/bin/alembic."
  echo "Create backend env first: python3 -m venv .venv && .venv/bin/pip install -r requirements && .venv/bin/pip install alembic psycopg2-binary"
  exit 1
fi
...
DATABASE_URL="$DATABASE_URL" "$ROOT_DIR/.venv/bin/alembic" upgrade head

echo "Database reset complete: $DB_NAME"
echo "All application records were cleared, including users."
