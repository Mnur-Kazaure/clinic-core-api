...
const STORE_DASHBOARD_ROLES = new Set([
  'PHARMACY_STORE_OFFICER',
]);

export default function PharmacyStoreLayout({
  children,
}: {
  ...
  useEffect(() => {
    async function verifyAccess() {
      try {
        ...
        if (!STORE_DASHBOARD_ROLES.has(user.role)) {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="mt-2 text-gray-600">Verifying pharmacy store access...</p>
        </div>
      </div>
    );
  }
